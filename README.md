...
* variables, parallel assignment, arrays, hashtables with literal and computed keys, `[pscustomobject]@{}` / `[ordered]@{}` literals, splatting, expandable strings with `$()` subexpressions, script blocks, functions, loops, `switch`, `try` / `catch` / `finally`, `throw`, `return`, `break`, and `continue`
* common PowerShell-style operators, including arithmetic, compound assignment, comparisons, wildcard/regex operators, script block call operator `&`, `-replace`, `-split`, `-join`, `-f`, `&&`, `||`, `$i++`, `$i--`, and `??`
